			...
			return Promise.all([
				fs.exec('adb', ['-s', device, 'shell', 'getprop']).then(function(result) {
					...
					var properties = {
						...
						'ro.build.version.security_patch': 'device_sec'
					};
					if (stderr) {
						deviceInfo['device-section'] = stderr.trim();
						return deviceInfo;
					} else {
				...
				fs.exec('adb', ['-s', device, 'shell', 'dumpsys', 'battery']).then(function(result) {
					...
					if (stderr) {
						batteryInfo['battery-section'] = stderr.trim();
						return batteryInfo;
					} else {
						var lines = stdout.split('\n');
						for (var i = 0; i < lines.length; i++) {
				...
				fs.exec('adb', ['-s', device, 'shell', 'uptime']).then(function(result) {
					...
					var stdout = result.stdout;
					if (stderr) {
						uptimeInfo['uptime-section'] = stderr.trim();
						return uptimeInfo;
					} else {
				...
				fs.exec('adb', ['-s', device, 'shell', 'uname', '-a']).then(function(result) {
					...
					var stdout = result.stdout;
					if (stderr) {
						kernelInfo['uname-section'] = stderr.trim();
						return kernelInfo;
					} else {
						var parts = stdout.trim().split(/\s+/);
				...
				fs.exec('adb', ['-s', device, 'shell', 'cat', '/proc/meminfo']).then(function(result) {
					...
					var stdout = result.stdout;
					if (stderr) {
						memmoryInfo['memory-section'] = stderr.trim();
						return memmoryInfo;
					} else {
						var parts = stdout.trim().split(/\s+/);
				...
				var uptimeInfo = results[2];
				var kernelInfo = results[3];
				var memmoryInfo = results[4];
				if (deviceInfo && batteryInfo && uptimeInfo && kernelInfo && memmoryInfo) {
					var mergedInfo = Object.assign({}, deviceInfo, batteryInfo, uptimeInfo, kernelInfo, memmoryInfo);
					return mergedInfo;
				} else {
		...
		var device = [
			...
			E('table', {'class': 'table cbi-section-table'}, [
				...
				E('tr', {'class': 'tr cbi-rowstyle-1'}, [
					...
					E('td', {'class': 'td'}, data.device_id || '-'),
					E('td', {'class': 'td left', 'width': '25%'}, _('Processors')),
					E('td', {'class': 'td'}, data.device_cpu ? (data.device_cpu.toUpperCase()) : '-')
				]),
				E('tr', {'class': 'tr cbi-rowstyle-2'}, [
					E('td', {'class': 'td left', 'width': '25%'}, _('Root status')),
					E('td', {'class': 'td'}, '-'),
					E('td', {'class': 'td left', 'width': '25%'}, _('Random Access Memory (RAM)')),
					E('td', {'class': 'td'}, data.device_memory || '-')
				]),
				E('tr', {'class': 'tr cbi-rowstyle-1'}, [
					E('td', {'class': 'td left', 'width': '25%'}, _('Brand name')),
					E('td', {'class': 'td'}, data.device_brand ? (data.device_brand.charAt(0).toUpperCase() + data.device_brand.slice(1)) : '-'),
					E('td', {'class': 'td left', 'width': '25%'}, _('Architecture')),
					E('td', {'class': 'td'}, data.device_uname ? (data.device_uname.arch) : '-')
				]),
				E('tr', {'class': 'tr cbi-rowstyle-2'}, [
					E('td', {'class': 'td left', 'width': '25%'}, _('Code name')),
					E('td', {'class': 'td'}, data.device_code || '-'),
					E('td', {'class': 'td left', 'width': '25%'}, _('Android version')),
					E('td', {'class': 'td'}, data.device_version || '-')
				]),
				E('tr', {'class': 'tr cbi-rowstyle-1'}, [
					E('td', {'class': 'td left', 'width': '25%'}, _('Model number')),
					E('td', {'class': 'td'}, data.device_model ? (data.device_model.charAt(0).toUpperCase() + data.device_model.slice(1)) : '-'),
					E('td', {'class': 'td left', 'width': '25%'}, _('SDK version')),
					E('td', {'class': 'td'}, data.device_sdk || '-')
				]),
				E('tr', {'class': 'tr cbi-rowstyle-2'}, [
					E('td', {'class': 'td left', 'width': '25%'}, _('Used time')),
					E('td', {'class': 'td'}, data.device_uptime || '-'),
					E('td', {'class': 'td left', 'width': '25%'}, _('Security patch level')),
					E('td', {'class': 'td'}, data.device_sec || '-')
				]),
				E('tr', {'class': 'tr cbi-rowstyle-1'}, [
					E('td', {'class': 'td left', 'width': '25%'}, _('System on Chip (SoC)')),
					E('td', {'class': 'td'}, data.device_soc ? (data.device_soc.toUpperCase()) : '-'),
					E('td', {'class': 'td left', 'width': '25%'}, _('Kernel version')),
					E('td', {'class': 'td'}, data.device_uname ? (data.device_uname.kernel) : '-')
				])
			])
		];
		var battery = [
			...
			E('table', {'class': 'table cbi-section-table'}, [
				...
				E('tr', {'class': 'tr cbi-rowstyle-1'}, [
					...
					E('td', {'class': 'td'}, data.battery_level || '-'),
				]),
				E('tr', {'class': 'tr cbi-rowstyle-2'}, [
					E('td', {'class': 'td left', 'width': '50%'}, _('Charge counter')),
					E('td', {'class': 'td'}, data.battery_counter || '-'),
				]),
				E('tr', {'class': 'tr cbi-rowstyle-1'}, [
					E('td', {'class': 'td left', 'width': '50%'}, _('Voltage')),
					E('td', {'class': 'td'}, data.battery_voltage || '-'),
				]),
				E('tr', {'class': 'tr cbi-rowstyle-2'}, [
					E('td', {'class': 'td left', 'width': '50%'}, _('Temperature')),
					E('td', {'class': 'td'}, data.battery_temperature || '-'),
				]),
				E('tr', {'class': 'tr cbi-rowstyle-1'}, [
					E('td', {'class': 'td left', 'width': '50%'}, _('Technology')),
					E('td', {'class': 'td'}, data.battery_technology || '-'),
				])
			])
		];
